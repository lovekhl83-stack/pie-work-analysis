...
$btnInstall.Add_Click({
    ...
    try {
        ...
        # 2. 파일 복사 — 인터넷 버전: MediaPipe(Pose) 자산은 CDN에서 받으므로 로컬 자산 불필요
        $lblStatus.Text = "[2/5] 프로그램 파일 복사 중..."
        $form.Refresh()
        $CopyItems = @('PIE.html','PIE_시작.bat','PIE_ST_server.ps1','PIE_ST_server_시작.bat','README.md','PIE_가이드.html')
        ...
        if ($chkDesktop.Checked) {
            ...
            $lnk.TargetPath = Join-Path $InstallPath 'PIE_시작.bat'
        ...
        if ($chkStartMenu.Checked) {
            ...
            $lnk2.TargetPath = Join-Path $InstallPath 'PIE_시작.bat'
        ...
        if ($result -eq "Yes") {
            Start-Process (Join-Path $InstallPath 'PIE_시작.bat') -WorkingDirectory $InstallPath
        }
        $form.Close()
    }
    catch {
